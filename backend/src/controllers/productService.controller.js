...
export const createProductService = async (req, res) => {
  try {
    ...
    if (!vendorId || !categoryId || !type || !title || price === undefined) {
      return res.status(400).json({ message: 'Vendor ID, Category ID, type, title, and price are required.' });
    }
    ...
    if (type === 'product' && Array.isArray(variants)) {
      ...
      return res.status(400).json({ message: 'Services cannot have variants.' });
    }
...
/**
 * @desc Get all products/services or filter by various criteria
...
export const getProductServices = async (req, res) => {
  try {
    const { vendorId, categoryId, type, pincode, title, minPrice, maxPrice } = req.query;
    const filter = {};
    ...
    if (categoryId) {
      if (!isValidObjectId(categoryId)) return res.status(400).json({ message: 'Invalid Category ID format.' });
    ...
    const productServices = await ProductService.find(filter).sort({ createdAt: -1 }); // Sort by newest first
...
export const getProductServiceById = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(id)) {
      return res.status(400).json({ message: 'Invalid Product/Service ID format.' });
    }
...
export const updateProductService = async (req, res) => {
  try {
    ...
    if (portfolio !== undefined) {
      if (!Array.isArray(portfolio)) return res.status(400).json({ message: 'Portfolio must be an array.' });
    ...
    if (effectiveType === 'product') {
      if (variants !== undefined) {
        if (!Array.isArray(variants)) return res.status(400).json({ message: 'Variants must be an array for products.' });
...
export const deleteProductService = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Product or Service deleted successfully.' });
  } catch (error) {
